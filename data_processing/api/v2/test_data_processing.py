...
def test_web_extraction_ok():
    url = f"{data_processing_url}/data_preprocessing/online"

    payload = json.dumps({
        "extractor": "web",
        "path": "https://mibluemedical.com/",
        "description": "Informacion general sobre la pagina de la empresa mi \
            blue medical"
    })
    headers = {
...
def test_web_extraction_invalidExt():
    ...
    payload = json.dumps({
        "extractor": "webdf",
        "path": "https://mibluemedical.com/blue-medical-horarios-ubicaciones/",
        "description": "Información sobre las clinicas o ubicaciones de la \
...
def test_web_extraction_invalidUrl():
    ...
    payload = json.dumps({
        "extractor": "web",
        "path": "mibluemedical.com/blue-medical-horarios-ubicaciones/",
        "description": "Información sobre las clinicas o ubicaciones de la \
            empresa blue medical"
    })
    headers = {
...
def test_file_extraction():
    url = f"{data_processing_url}/data_preprocessing/file"
    path = Path(__file__).parent.parent.parent.parent.parent / "bluemeds.docx"

    payload = {"params": json.dumps({
        "extractor": "docx",
        "description": "Informacion sobre el servicio bluemeds ofrecido por \
            la empresa blue medical"
        })}
...
def test_file_extraction_invalidExt():
    ...
    assert response.status_code == 422
    assert details['detail'][0]['msg'] == \
        "value is not a valid enumeration member; permitted: 'docx', 'pdf', 'txt'"
    assert details['detail'][0]['type'] == "type_error.enum"
